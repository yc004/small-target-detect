...
% 基于 docs/作品报告模板.docx 复刻
% 编译: xelatex report.tex
% ═══════════════════════════════════════════════════════════════════════════════

\documentclass[12pt,a4paper,fontset=none]{ctexart}

% ─── ctex numbering: 第一章、1.1、1.1.1 ──────────────────────────────────
\ctexset{
    section = {
        name = {第,章},
...
% ─── Fonts (macOS system fonts, matching docx 华文中宋/华文楷体/宋体/黑体) ───
...
\setCJKfamilyfont{zhzongsong}{Songti SC}  % macOS 无华文中宋，用宋体替代
...
% ─── Heading spacing (match docx EMU values) ──────────────────────────────────
% Heading 1: space_before=594360 EMU ≈ 46.8pt, space_after=99060 EMU ≈ 7.8pt
\titleformat{\section}{\centering\heiti\bfseries\Large}{}{0em}{}
\titlespacing{\section}{0pt}{47pt}{8pt}

% Heading 2: space_before=297180 EMU ≈ 23.4pt, space_after=59055 EMU ≈ 4.7pt
\titleformat{\subsection}{\heiti\bfseries\large}{}{0em}{}
\titlespacing{\subsection}{0pt}{23pt}{5pt}

% Heading 3: space_before=198120 EMU ≈ 15.6pt, space_after=39370 EMU ≈ 3.1pt
\titleformat{\subsubsection}{\heiti\bfseries\normalsize}{}{0em}{}
\titlespacing{\subsubsection}{0pt}{16pt}{3pt}

% ─── Body text ────────────────────────────────────────────────────────────────
% 正文段落: first_line_indent=304800 EMU ≈ 24pt (2 chars), space_before=after=39370 EMU ≈ 3pt
\setlength{\parindent}{2em}       % first-line indent 2 chars
\setlength{\parskip}{3pt plus1pt minus1pt}

% ─── Caption style ─────────────────────────────────────────────────────────────
% Caption: space_before=59055, space_after=59055 EMU
\captionsetup[figure]{font={small,bf}, belowskip=5pt, aboveskip=5pt}
\captionsetup[table]{font={small,bf}, belowskip=5pt, aboveskip=5pt}
...
% 华文中宋 28pt Bold（macOS 用宋体 Bold 替代）
...
% 宋体 16pt 左对齐
\begin{flushleft}
{\songti\fontsize{16pt}{28pt}\selectfont
作品编号：\underline{\hspace{6cm}}\\[12pt]
作品名称：\underline{\hspace{6cm}}\\[12pt]
填写日期：\underline{\hspace{6cm}}
}
\end{flushleft}
...
本作品以 YOLOv8s 为基线检测框架，采用四阶段递进式改进策略（图~\ref{fig:pipeline}），构建面向小目标交通标志的实时检测系统。四个改进阶段分别为：Stage 2 增加 P2 高分辨率检测层，Stage 3 引入 ECA 通道注意力，Stage 4 采用 Copy-Paste 数据增强，Stage 5 集成 Soft-NMS 后处理。
...
YOLOv8s 默认检测头包含 P3（下采样 8 倍，$80\times 80$）、P4（$40\times 40$）、P5（$20\times 20$）三层。本作品在 Neck 的 FPN 路径中增加一层上采样，将 P3 特征进一步放大至 P2 级别（下采样 4 倍，$160\times 160$），连接 Backbone 第 2 层（stride=4）特征经 C2f 融合后，构成 P2 检测头。P2 的每个空间位置对应 $4\times 4$ 像素区域，$16\times 16$ 的小目标在该层占据 $4\times 4$ 个特征点（P3 仅 $2\times 2$），特征表达能力翻倍。

改进后的检测头结构如图~\ref{fig:p2} 所示。P2 模型参数量由 11.1M 增至 14.5M，FLOPs 由 28.7G 增至 47.5G。
...
训练时以 0.5 概率执行：从同 batch 另一张图中随机选取小目标框（面积 $< 32^2$ px），复制像素并随机缩放 0.8--1.2 倍，粘贴到当前图的空旷区域（与现有框 IoU $< 0.1$），同步更新标注。该策略可有效增加尾部类别的正样本密度。
...
本作品采用 TT100K 2021 版数据集，原始数据包含约 100,000 张腾讯街景全景图，标注了约 30,000 个交通标志实例，涵盖 221 类中国交通标志。筛选其中 5 类典型小目标标志进行实验：
...
数据集按 7:2:1 划分为训练集（2171 张）、验证集（622 张）和测试集（314 张）。训练时下采样至 $640\times 640$，标注格式由原始 TT100K JSON（含嵌套 objects 结构）转换为 YOLO 归一化格式。
...
\begin{enumerate}[label=(\arabic*)]
    ...
    \item \textbf{工程化设计}：YAML 配置驱动 + CLI 覆写，支持 Linux/Windows/Mac 一键训练。
\end{enumerate}

\subsection{应用推广}

\begin{itemize}
    \item \textbf{车载辅助驾驶}：嵌入车载平台，实时提供交通标志提示；
    \item \textbf{交通监控}：部署于路边摄像头，自动检测并统计标志状态；
    \item \textbf{高精地图更新}：结合 SLAM，批量采集道路标志信息。
\end{itemize}

\subsection{作品展望}

\begin{enumerate}[label=(\arabic*)]
    \item \textbf{模型轻量化}：探索知识蒸馏或模型剪枝，降低 P2 模型计算开销，达到边缘部署要求；
    \item \textbf{多天气鲁棒性}：引入雾天、夜间数据增强或域自适应方法；
    \item \textbf{多帧融合}：利用视频时序信息进行目标跟踪，减少单帧漏检；
    \item \textbf{端到端部署}：导出 ONNX / TensorRT，实现边缘设备实时推理。
\end{enumerate}
