...
|------|------|
| 任务 | 小目标交通标志检测与识别 |
| 数据集 | TT100K (2021) — 201 类交通标志 |
| 基线模型 | YOLO26s (ultralytics 2025) |
| 改进点 | P2 检测层 → ARF-Head 自适应感受野检测头 |
| 原创模块 | ARF-Head（自适应感受野检测头） |
| 框架 | PyTorch + Ultralytics |

### 数据集

使用 TT100K 2021 版全部 201 类交通标志，涵盖限速、禁令、指示、警告等大类，约 30,000 个标注实例。训练 / 验证 / 测试按 7:2:1 划分。
...
| Stage | 改进 | 创新程度 | mAP@0.5 | mAP@0.5:0.95 | Recall | 状态 |
|-------|------|:---:|:---:|:---:|:---:|------|
| 1 | 基线 YOLO26s | — | 0.544 | 0.408 | 0.503 | ✅ |
| 2 | + P2 高分辨率检测层 | 官方 | 0.552 | 0.425 | 0.504 | ✅ |
| 3 | + **ARF-Head** 自适应感受野检测头 | 原创 | 0.548 | 0.421 | **0.519** | ✅ |

> 受限于硬件算力，项目止于 Stage 3。BCEM 和 HJ-Loss 模块已完成代码实现，留待后续训练。
